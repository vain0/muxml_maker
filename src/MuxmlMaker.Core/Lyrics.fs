...
module Lyrics =
  ...
  let inline tryCompleteTimeTags
    ...
    [ for (taggedLine, prevRTag, nextLTag) in ottlPrevNext do
        ...
        if lTag >= rTag then
          failwithf "Non-positive interval: %s" (string line)
    ...
    | WithTimeTag ttl ->
      ...
      [
        for it in ttl |> pairwise id do
          ...
          // 行間の時間差を処理
          if previousMargin >= marginThreshold then
            yield (None, Interval previousMargin)
  ...
  let toTimeTagged =
    ...
    | WithInterval self ->
      self
      |> List.fold
        (fun (acc, total) (line, Interval interval) ->
